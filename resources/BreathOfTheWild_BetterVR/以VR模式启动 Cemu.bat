...
rem 1) 硬性要求：Cemu.exe 必须位于脚本旁边
if not exist "%CEMU_EXE%" (
  call :Log "错误：在脚本旁边未找到 Cemu.exe。正在退出。"
  call :Popup "在此脚本旁边未找到 Cemu.exe。请将此 .bat 文件放置在 Cemu.exe 旁边（或以 Cemu 的文件夹作为工作目录启动），然后重试。" "BetterVR"
  exit /b 1
)

rem 1.5) 检查 Cemu 版本（需要 2.6+）
...
if errorlevel 1 (
  ...
  call :Popup "警告：您似乎正在使用旧版本的 Cemu。BetterVR 专为 Cemu 2.6 或更新版本设计。您可能会遇到问题。" "BetterVR 警告"
)
...
if exist "%CEMU_DIR%\portable\" if not exist "%CEMU_DIR%\portable\graphicPacks\" (
  call :Log "注意：portable 存在但 portable\graphicPacks 缺失。用户可能手动删除了 graphicPacks 文件夹。"
)
goto CheckDone

:CheckSettings
if exist "%CEMU_DIR%\settings.xml" if not exist "%CEMU_DIR%\graphicPacks\" (
  call :Log "注意：settings.xml 存在但 graphicPacks 缺失。用户可能手动删除了 graphicPacks 文件夹。"
)
goto CheckDone
...
if "%FOUND_INSTALL%"=="1" (
  call :Log "信息：在某处发现了现有的 BreathOfTheWild_BetterVR，但源资源包文件夹缺失。保持现有安装不变。"
  goto LaunchCemu
)

call :Log "错误：在任何已知位置均未找到源资源包文件夹，也未找到现有安装。"
call :Popup "在 Cemu.exe 旁边未找到 BreathOfTheWild_BetterVR 文件夹，且其未安装在任何已知的 graphicPacks 位置。请将 BreathOfTheWild_BetterVR 文件夹放在 Cemu.exe 旁边，然后重试。" "BetterVR"
exit /b 2
...
if not exist "%TARGET_BASE%\" (
  ...
  if errorlevel 1 (
    ...
    call :Popup "无法创建 graphicPacks 文件夹。请检查权限并重试。" "BetterVR"
    exit /b 3
  )
  call :Log "已创建目标基础文件夹。"
)

rem 5) 删除目标中现有的资源包文件夹（删除并替换）
set "TARGET_PACK=%TARGET_BASE%\BreathOfTheWild_BetterVR"
if exist "%TARGET_PACK%\" (
  call :Log "信息：正在删除现有的目标资源包文件夹：%TARGET_PACK%"
  rmdir /s /q "%TARGET_PACK%" 2>nul
)

rem 6) 安装（移动语义）
rem 首先尝试 robocopy /MOVE。如果失败且代码严重 (8+)，则重试“复制后删除”。
call :InstallPack "%SOURCE_PACK%" "%TARGET_PACK%"
if %RC% GEQ 8 (
  call :Log "错误：安装失败 (robocopy RC=%RC%)。调试日志保留在：%ROBOLOG%"
  call :Popup "无法安装 BreathOfTheWild_BetterVR (robocopy RC=%RC%)。请打开 Cemu.exe 旁边的 robocopy_debug.log 查看确切原因。" "BetterVR"
  exit /b 4
)
...
if "%ERRORLEVEL%"=="0" (
  ...
  call :Popup "警告：UAC 已禁用。这将强制 Cemu 以管理员身份运行，可能会破坏 VR 功能。尝试以降低的权限启动。" "BetterVR 警告"
) else (
  call :Log "信息：UAC 已启用。"
)
...
call :Log "信息：正在将资源包从 '%SRC%' 安装到 '%DST%'"

rem 在安装尝试开始时删除任何旧的调试日志
del /q "%ROBOLOG%" 2>nul

call :Log "信息：Robocopy 第 1 遍：移动 (MOVE)"
robocopy "%SRC%" "%DST%" /E /MOVE /R:0 /W:0 /V /LOG:"%ROBOLOG%" >nul
set "RC=%errorlevel%"
call :Log "信息：Robocopy 移动 RC=%RC%"

if %RC% LSS 8 goto InstallSuccess

call :Log "警告：Robocopy 移动失败 (RC=%RC%)。重试为“复制后删除”。"
...
call :Log "信息：Robocopy 第 2 遍：复制 (COPY)"
robocopy "%SRC%" "%DST%" /E /R:0 /W:0 /V /LOG+:"%ROBOLOG%" >nul
set "RC=%errorlevel%"
call :Log "信息：Robocopy 复制 RC=%RC%"
...
rem 优先使用 PowerShell MessageBox，回退到 mshta 弹窗
